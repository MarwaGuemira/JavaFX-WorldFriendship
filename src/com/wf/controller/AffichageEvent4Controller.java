 ...
 * @author Marwa
 */
public class AffichageEvent4Controller implements Initializable {
...
//        stat.setData(list);
       ...
       try {
         ...
         int i = MeseventactuelController.id2;
...
//            OB3.add(Integer.toString(ee.getUser_id().getId()));}
    ...
    int ki = p.getNbrdislikes();
    int j = p.getNbrlikes();
    List<Integer> nlike = new ArrayList<>();
    nlike.add(j);
    nlike.add(ki);
      ...
      for(Integer e:nlike) {
            list.addAll(
                new PieChart.Data(String.valueOf(e), e)             
        );
        }
        stat.setAnimated(true);
        stat.setData(list);
...
//        for (Evenement e : evee) {
       ...
       // }
      int in = MeseventactuelController.id2;
